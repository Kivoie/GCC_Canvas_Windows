...
ECHO.
ECHO ==================== MENU ====================
ECHO [1] Compile .c file to executable     [6] List all files within all subdirectories
ECHO [2] Run an executable                 [7] Help
ECHO [3] Compile .c file and run executable[8] Open a c/cpp file in notepad++
...
IF EXIST %gccFile% gcc %gccFile% -o %gccFile%.exe && ECHO. && ECHO -------- Running '%gccFile%'. . . -------- && %gccFile%.exe && ECHO -------- '%gccFile%' has completed its runtime. -------- && GOTO CRLOG
...
IF EXIST %gccCompile% gcc %gccCompile% -o %gccCompile%.exe && ECHO '%gccCompile%' has compiled successfully. && GOTO CLOG
